            ...
            try {
            	foreach ($listaRecords as $listaRecord) {
                    ...
                    if (json_last_error() === JSON_ERROR_NONE) {
						if (isset($record['id_activo_entrega'])) {
							$model=ActivoEntrega020103::model()->deleteByPk($record['id_activo_entrega']);
                           	$audi=new LogSistema030003();
	                		if ($model==1) {
                            	$audi->insertAudi("delete",ActivoEntrega020103::model()->tableName(),$record['id_activo_entrega']);
	                    		$contValRecords++;
	                    	} else {
