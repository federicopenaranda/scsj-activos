            ...
            try {
            	foreach ($listaRecords as $listaRecord) {
                    ...
                    if (json_last_error() === JSON_ERROR_NONE) {
						if (isset($record['id_activo_proveedor'])) {
							$model=ActivoProveedor020104::model()->deleteByPk($record['id_activo_proveedor']);
                           	$audi=new LogSistema030003();
	                		if ($model==1) {
                            	$audi->insertAudi("delete",ActivoProveedor020104::model()->tableName(),$record['id_activo_proveedor']);
	                    		$contValRecords++;
	                    	} else {
