            ...
            try {
            	foreach ($listaRecords as $listaRecord) {
                    ...
                    if (json_last_error() === JSON_ERROR_NONE) {
						if (isset($record['id_activo_estado'])) {
							$model=ActivoEstado020204::model()->deleteByPk($record['id_activo_estado']);
                           	$audi=new LogSistema030003();
	                		if ($model==1) {
                            	$audi->insertAudi("delete",ActivoEstado020204::model()->tableName(),$record['id_activo_estado']);
	                    		$contValRecords++;
	                    	} else {
