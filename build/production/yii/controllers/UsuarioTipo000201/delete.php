            ...
            try {
            	foreach ($listaRecords as $listaRecord) {
                    ...
                    if (json_last_error() === JSON_ERROR_NONE) {
						if (isset($record['id_usuario_tipo'])) {
							$model=UsuarioTipo000201::model()->deleteByPk($record['id_usuario_tipo']);
                           	$audi=new LogSistema030003();
	                		if ($model==1) {
                            	$audi->insertAudi("delete",UsuarioTipo000201::model()->tableName(),$record['id_usuario_tipo']);
	                    		$contValRecords++;
	                    	} else {
