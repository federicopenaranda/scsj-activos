            ...
            try {
            	foreach ($listaRecords as $listaRecord) {
                    ...
                    if (json_last_error() === JSON_ERROR_NONE) {
						if (isset($record['id_revision'])) {
							$model=Revision020204::model()->deleteByPk($record['id_revision']);
                           	$audi=new LogSistema030003();
	                		if ($model==1) {
                            	$audi->insertAudi("delete",Revision020204::model()->tableName(),$record['id_revision']);
	                    		$contValRecords++;
	                    	} else {
