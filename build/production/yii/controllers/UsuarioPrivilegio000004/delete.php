            ...
            try {
            	foreach ($listaRecords as $listaRecord) {
                    ...
                    if (json_last_error() === JSON_ERROR_NONE) {
						if (isset($record['id_usuario_privilegio'])) {
							$model=UsuarioPrivilegio000004::model()->deleteByPk($record['id_usuario_privilegio']);
                           	$audi=new LogSistema030003();
	                		if ($model==1) {
                            	$audi->insertAudi("delete",UsuarioPrivilegio000004::model()->tableName(),$record['id_usuario_privilegio']);
	                    		$contValRecords++;
	                    	} else {
