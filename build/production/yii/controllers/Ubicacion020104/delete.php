            ...
            try {
            	foreach ($listaRecords as $listaRecord) {
                    ...
                    if (json_last_error() === JSON_ERROR_NONE) {
						if (isset($record['id_ubicacion'])) {
							$model=Ubicacion020104::model()->deleteByPk($record['id_ubicacion']);
                           	$audi=new LogSistema030003();
	                		if ($model==1) {
                            	$audi->insertAudi("delete",Ubicacion020104::model()->tableName(),$record['id_ubicacion']);
	                    		$contValRecords++;
	                    	} else {
