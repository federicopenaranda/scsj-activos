            ...
            try {
            	foreach ($listaRecords as $listaRecord) {
                    ...
                    if (json_last_error() === JSON_ERROR_NONE) {
						if (isset($record['id_activo_valor'])) {
							$model=ActivoValor020103::model()->deleteByPk($record['id_activo_valor']);
                           	$audi=new LogSistema030003();
	                		if ($model==1) {
                            	$audi->insertAudi("delete",ActivoValor020103::model()->tableName(),$record['id_activo_valor']);
	                    		$contValRecords++;
	                    	} else {
