            ...
            try {
            	foreach ($listaRecords as $listaRecord) {
                    ...
                    if (json_last_error() === JSON_ERROR_NONE) {
						if (isset($record['id_reporte'])) {
							$model=Reporte030004::model()->deleteByPk($record['id_reporte']);
                           	$audi=new LogSistema030003();
	                		if ($model==1) {
                            	$audi->insertAudi("delete",Reporte030004::model()->tableName(),$record['id_reporte']);
	                    		$contValRecords++;
	                    	} else {
