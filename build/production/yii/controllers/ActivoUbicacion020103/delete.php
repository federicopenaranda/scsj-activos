            ...
            try {
            	foreach ($listaRecords as $listaRecord) {
                    ...
                    if (json_last_error() === JSON_ERROR_NONE) {
						if (isset($record['id_activo_ubicacion'])) {
							$model=ActivoUbicacion020103::model()->deleteByPk($record['id_activo_ubicacion']);
                           	$audi=new LogSistema030003();
	                		if ($model==1) {
                            	$audi->insertAudi("delete",ActivoUbicacion020103::model()->tableName(),$record['id_activo_ubicacion']);
	                    		$contValRecords++;
	                    	} else {
