            ...
            try {
				foreach ($listaRecords as $listaRecord) {
                    ...
                    if (json_last_error() === JSON_ERROR_NONE) {
                    	try{
        					$error.= (!isset($record['id_activo_ubicacion'])) ? "Variable indefinida {id_activo_ubicacion}" : "";
							...
							if ($error == "") {
								$model=ActivoUbicacion020103::model()->findByPk($record['id_activo_ubicacion']);							
								...
								if ($model!==null) {
		                            ...
		                            if ($model->validate()) {
		                                $model->save();
                                        $audi->insertAudi("update",$model->tableName(),$record['id_activo_ubicacion']);
		                                $contValRecords++;
		                            } else {
