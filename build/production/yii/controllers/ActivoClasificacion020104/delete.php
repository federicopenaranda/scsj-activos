            ...
            try {
            	foreach ($listaRecords as $listaRecord) {
                    ...
                    if (json_last_error() === JSON_ERROR_NONE) {
						if (isset($record['id_activo_clasificacion'])) {
							$model=ActivoClasificacion020104::model()->deleteByPk($record['id_activo_clasificacion']);
                           	$audi=new LogSistema030003();
	                		if ($model==1) {
                            	$audi->insertAudi("delete",ActivoClasificacion020104::model()->tableName(),$record['id_activo_clasificacion']);
	                    		$contValRecords++;
	                    	} else {
