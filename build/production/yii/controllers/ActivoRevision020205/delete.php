            ...
            try {
            	foreach ($listaRecords as $listaRecord) {
                    ...
                    if (json_last_error() === JSON_ERROR_NONE) {
						if (isset($record['id_activo_revision'])) {
							$model=ActivoRevision020205::model()->deleteByPk($record['id_activo_revision']);
                           	$audi=new LogSistema030003();
	                		if ($model==1) {
                            	$audi->insertAudi("delete",ActivoRevision020205::model()->tableName(),$record['id_activo_revision']);
	                    		$contValRecords++;
	                    	} else {
