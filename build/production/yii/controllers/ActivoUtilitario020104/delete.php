            ...
            try {
            	foreach ($listaRecords as $listaRecord) {
                    ...
                    if (json_last_error() === JSON_ERROR_NONE) {
						if (isset($record['id_activo_utilitario'])) {
							$model=ActivoUtilitario020104::model()->deleteByPk($record['id_activo_utilitario']);
                           	$audi=new LogSistema030003();
	                		if ($model==1) {
                            	$audi->insertAudi("delete",ActivoUtilitario020104::model()->tableName(),$record['id_activo_utilitario']);
	                    		$contValRecords++;
	                    	} else {
