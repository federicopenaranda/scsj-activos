...
p.fk_id_beneficiario = ".$id;
	...
	*/
	/*
	* •	Reporte en PDF de Proveedor con todos sus datos:
	*/
	
	/*
	* Reporte en PDF1 de un activo y todos sus datos e historia que se divide en 3 tablas donde el id es el id activo
	* PARTE 2
	*/
	public function queryPdf_12_de_rep1($id) {
		$sql="SELECT
u.primer_nombre_usuario,
u.apellido_paterno_usuario,
u.apellido_materno_usuario,
ae.fecha_activo_entrega
FROM
activo_02_01_01 AS a
INNER JOIN activo_entrega_02_01_03 AS ae ON ae.fk_id_activo = a.id_activo
INNER JOIN usuario_00_01_01 AS u ON ae.fk_id_usuario_recibio = u.id_usuario AND ae.fk_id_usuario_entrego = u.id_usuario
WHERE
a.id_activo = $id
GROUP BY
ae.fecha_activo_entrega DESC";
        $rows=Yii::app()->db->createCommand($sql)
        ->queryAll();
        return $rows;
	}
	...
	* •	Reporte en PDF de Proveedor con todos sus datos:
	*/
	public function consulta11_de_rep2($id) {
		$sql="SELECT
...
a.id_activo = ".$id;
	...
	public function consulta11_de_rep1($fk_id,$id_rev) {
		$sql="SELECT
a.codigo_activo,
...
a.fecha_baja_activo,
`at`.nombre_activo_tipo,
`at`.descripcion_activo_tipo,
...
INNER JOIN activo_tipo_02_01_04 AS `at` ON a.fk_id_activo_tipo = `at`.id_activo_tipo
...
INNER JOIN activo_entrega_02_01_03 AS ae ON ae.fk_id_usuario_recibio = u.id_usuario AND ae.fk_id_usuario_entrego = u.id_usuario
WHERE
ae.fk_id_usuario_recibio = ".$fk_id;
        $rows=Yii::app()->db->createCommand($sql)
        ->queryAll();
...
INNER JOIN revision_02_02_04 AS r ON ar.fk_id_revision = r.id_revision
WHERE
ae.valor_activo_estado = 'baja' AND
...
INNER JOIN revision_02_02_04 AS r ON ar.fk_id_revision = r.id_revision
WHERE
ae.valor_activo_estado = 'alta' AND
r.id_revision = $id AND
